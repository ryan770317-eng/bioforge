...
const SUPPLEMENT_GROUPS: SupplementGroup[] = [
  {
    ...
    items: [
      {
        id: "breakfast-mvm",
        name: "Garden of Life mykind Organics Men's Once Daily",
        dose: "1 顆",
        note: "MVM 主軸，植化素來源",
      },
      {
        id: "breakfast-adrenal",
        name: "Thorne Adrenal Complex",
        dose: "1 顆",
        note: "腎上腺調節（第一顆）",
      },
      {
        id: "breakfast-glutamine",
        name: "L-Glutamine Powder（Doctor's Best）",
        dose: "5g 入水",
        note: "腸黏膜修復，勿空腹",
      },
    ],
  },
  {
    ...
    items: [
      {
        id: "dinner-spectrazyme",
        name: "SpectraZyme Complete",
        dose: "1 顆（餐前）",
        note: "廣效消化酵素",
      },
      {
      ...
      {
        id: "dinner-vitc",
        name: "Life Extension Vit C + Bio-Quercetin",
        dose: "1 顆",
        note: "抗炎＋腸道緊密連結修復",
      },
    ],
              ...
              <ul className="space-y-2">
                {group.items.map((item) => {
                  ...
                  return (
                    <li
                      key={item.id}
                      className={`rounded-2xl px-4 py-3 shadow-sm flex items-start gap-3 transition-colors ${
                        done ? "bg-[#6B9E780D]" : "bg-white"
                      }`}
                    >
                      {/* Check button */}
                      <button
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        <p className={`text-xs mt-0.5 ${done ? "text-[#8B7D6B]/60" : "text-[#D4A24E]"}`}>
                          {item.dose}
                        </p>
                        <p className={`text-xs mt-0.5 ${done ? "text-[#8B7D6B]/60" : "text-[#8B7D6B]"}`}>
                          {item.note}
                        </p>
                      </div>
                    </li>
